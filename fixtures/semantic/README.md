...
`three-column-flow.pdf` is generated by `scripts/generate-three-column-fixture.mjs` with the
`@boxpdf/writer` DSL. It deliberately paints horizontal bands in row-major source order while two
sustained vertical whitespace gutters require column-major semantic reading order. A bordered table
and narrow footer follow the columns to verify that traversal ends without shredding rows or
hoisting page furniture.
